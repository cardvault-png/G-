...
export const mockPrisma = {
  user: {
    findUnique: async ({ where }: any) => {
      ...
      if (where.phone) {
        for (const user of db.users.values()) {
          if (user.phone === where.phone) return user;
        }
        return null;
      }
      if (where.username) {
    ...
    findFirst: async ({ where }: any) => {
      if (where.referralCode) {
        for (const user of db.users.values()) {
          if (user.referralCode === where.referralCode) return user;
        }
        return null;
      }
      return null;
    ...
    findMany: async ({ where, orderBy, take }: any = {}) => {
      ...
      if (where?.kycStatus) results = results.filter(u => u.kycStatus === where.kycStatus);
      if (where?.isBanned !== undefined) results = results.filter(u => u.isBanned === where.isBanned);
    ...
    update: async ({ where, data }: any) => {
      const user = await mockPrisma.user.findUnique({ where });
    ...
    count: async ({ where }: any = {}) => {
      ...
      if (where?.role) count = Array.from(db.users.values()).filter(u => u.role === where.role).length;
      return count;
    },
  },
  ...
  wallet: {
    findUnique: async ({ where }: any) => {
      if (where.id) return db.wallets.get(where.id) || null;
      return null;
    },
    findFirst: async ({ where }: any) => {
      for (const wallet of db.wallets.values()) {
        if (wallet.userId === where.userId && wallet.type === where.type) return wallet;
      }
      return null;
    },
    findMany: async ({ where }: any = {}) => {
      let results = Array.from(db.wallets.values());
      if (where?.userId) results = results.filter(w => w.userId === where.userId);
      return results;
    },
    create: async ({ data }: any) => {
      const id = generateId();
    ...
    createMany: async ({ data }: any) => {
      for (const item of data) {
        await mockPrisma.wallet.create({ data: item });
      }
      return { count: data.length };
    },
    update: async ({ where, data }: any) => {
      const wallet = await mockPrisma.wallet.findUnique({ where });
  ...
  otpCode: {
    ...
    findMany: async ({ where }: any = {}) => {
      ...
      if (where?.identifier) results = results.filter(o => o.identifier === where.identifier);
      if (where?.usedAt === null) results = results.filter(o => !o.usedAt);
      return results;
    },
    create: async ({ data }: any) => {
      const id = generateId();
  ...
  loginAttempt: {
    ...
    findMany: async ({ where, orderBy, take }: any = {}) => {
      ...
      if (where?.ipAddress) results = results.filter(a => a.ipAddress === where.ipAddress);
      if (where?.success === false) results = results.filter(a => !a.success);
      if (orderBy?.createdAt === 'desc') results.sort((a, b) => b.createdAt - a.createdAt);
      if (take) results = results.slice(0, take);
      return results;
    },
  ...
  transaction: {
    findUnique: async ({ where }: any) => {
      if (where.id) return db.transactions.get(where.id) || null;
      return null;
    },
    findMany: async ({ where, orderBy, take, skip }: any = {}) => {
      ...
      if (where?.walletId) results = results.filter(t => t.walletId === where.walletId);
      if (where?.status) results = results.filter(t => t.status === where.status);
      if (where?.type) results = results.filter(t => t.type === where.type);
      if (orderBy?.createdAt === 'desc') results.sort((a, b) => b.createdAt - a.createdAt);
    ...
    update: async ({ where, data }: any) => {
      const transaction = await mockPrisma.transaction.findUnique({ where });
    ...
    count: async ({ where }: any = {}) => {
      ...
      if (where?.status) count = Array.from(db.transactions.values()).filter(t => t.status === where.status).length;
      if (where?.userId) count = Array.from(db.transactions.values()).filter(t => t.userId === where.userId).length;
      return count;
    },
  },
  ...
  giftCard: {
    findUnique: async ({ where }: any) => {
      if (where.id) return db.giftCards.get(where.id) || null;
      return null;
    },
    findMany: async ({ where, orderBy, take }: any = {}) => {
      let results = Array.from(db.giftCards.values());
      if (where?.userId) results = results.filter(g => g.userId === where.userId);
      if (where?.status) results = results.filter(g => g.status === where.status);
      if (orderBy?.createdAt === 'desc') results.sort((a, b) => b.createdAt - a.createdAt);
      if (take) results = results.slice(0, take);
      return results;
    },
    ...
    update: async ({ where, data }: any) => {
      const giftCard = await mockPrisma.giftCard.findUnique({ where });
    ...
    count: async ({ where }: any = {}) => {
      ...
      if (where?.status) count = Array.from(db.giftCards.values()).filter(g => g.status === where.status).length;
      return count;
    },
  },
  ...
  notification: {
    findMany: async ({ where, orderBy, take }: any = {}) => {
      ...
      if (where?.isRead === false) results = results.filter(n => !n.isRead);
      if (orderBy?.createdAt === 'desc') results.sort((a, b) => b.createdAt - a.createdAt);
      if (take) results = results.slice(0, take);
      return results;
    },
    ...
    count: async ({ where }: any = {}) => {
      ...
      if (where?.userId) count = Array.from(db.notifications.values()).filter(n => n.userId === where.userId).length;
      if (where?.isRead === false) count = Array.from(db.notifications.values()).filter(n => !n.isRead).length;
      return count;
    },
  },
  ...
  cryptoRate: {
    findUnique: async ({ where }: any) => {
      if (where.symbol) return db.cryptoRates.get(where.symbol) || null;
      return null;
    },
    findMany: async () => {
    ...
    upsert: async ({ where, create, update }: any) => {
      const existing = await mockPrisma.cryptoRate.findUnique({ where });
      if (existing) {
    ...
    findUnique: async ({ where }: any) => {
      if (where.adminId) {
        for (const wallet of db.adminWallets.values()) {
          if (wallet.adminId === where.adminId) return wallet;
        }
      }
      return null;
    },
  ...
  $clear: () => {
    for (const key of Object.keys(db)) {
      db[key as keyof typeof db].clear();
    }
  },
...
export const seedDefaultData = async () => {
  ...
  if (!admin) {
    ...
    admin = await mockPrisma.user.create({
      data: {
    ...
    // Create admin wallet
    await mockPrisma.adminWallet.create({
      data: {
        adminId: admin.id,
        balance: 15000,
  ...
  for (const rate of defaultRates) {
    const existing = await mockPrisma.cryptoRate.findUnique({ where: { symbol: rate.symbol } });
    if (!existing) {
      await mockPrisma.cryptoRate.upsert({
