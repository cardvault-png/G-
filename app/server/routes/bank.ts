...
router.get('/banks/:country', authenticate, asyncHandler(async (req, res) => {
...
// Verify bank account
router.post('/verify-account', authenticate, asyncHandler(async (req, res) => {
...
router.post('/accounts', authenticate, asyncHandler(async (req, res) => {
  ...
  if (!country || !bankName || !accountNumber || !accountName) {
    throw new AppError('Country, bank name, account number, and account name are required', 400);
  }
  ...
  if (existing) {
    throw new AppError('Bank account already exists', 409);
  }
...
// Update bank account
router.patch('/accounts/:id', authenticate, asyncHandler(async (req, res) => {
  const { id } = req.params;
